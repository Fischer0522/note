...
![2022-07-17-20-03-55-8966773b5370b0c514d019c9dcc4898.jpg](mysql45讲.assets/93a7294dd9ea8dc1016f69c0fd223f0387cf1067.jpg) 

![img](mysql45讲.assets/b7f27917ceb0be90ef7b201f2794c817.png)

在条件全部写在join 的on内时，先将驱动表的全部数据导入join_buffer再通过on 进行匹配，如果匹配成功则填入数值，如果无匹配结果则通过null填充

![img](mysql45讲.assets/f5712c56dc84d331990409a5c313ea9c.png)

使用 NLJ算法，将b中的值对应到表a中查询，匹配到记录后在判断where的条件，如果不满则则被过滤掉，因此不会存在null的情况

由于where的语义为匹配成功就放弃，与join 的语义一致，因此优化器会将left join优化为join,因此，left join的左边不一定为驱动表
...
- 组织方式：(堆组织表)数据和索引分开，数据以数组的形式存放，按照插入的顺序，删除之后会留下空位，后续新插入的可以利用之前的空位，索引通过哈希表的方式组织，InnoDB数据放在主键索引上的组织方式称为索引组织表
- 索引：通过哈希表的形式组织，并且所有索引的地位都相同，不存在回表的过程，数据**位置**发生变化是，所有的索引都需要进行修改
- 数据：内存表不支持 Blob 和 Text 字段，并且即使定义了 varchar(N)，实际也当作 char(N)，也就是固定长度字符串来存储，因此内存表的每行数据长度相同。

Memory引擎可以手动指定索引为btree,与MySQL情况相同

```mysql
alter table t1 add index a_btree_index using btree (id);
```

![img](mysql45讲.assets/1788deca56cb83c114d8353c92e3bde3.jpg)
...
重启后会表会被清空，主要在主库和备库之间的同步存在一定的问题

- MS架构下，如果备库重启了，主库再操作内存表，则会在备库上找不到对应的表
- MM架构下，备库重启，会向备库的binlog写入一条`delete from t`,如果再发生主备切换，主库再将备库的binlog拿出来执行，此时导致主库上面还存在的内存表再切换时被清空

建议除了在数据量可控，不会耗费过多内存的情况下，考虑使用内存表。其他时候全部使用InnoDB,由于InnoDB的buffer pool,因此性能也不会过差，并且InnoDB 在并发度上的支持度更好

### 自增主键

使用InnoDB引擎时，自增值保存在内存当中，并未持久化，每次重启后都寻找自增值的最大值，将最大值+1作为当前的自增值，8.0版本之后，自增值保存在redolog当中，有了持久化的能力

**自增修改机制**：

- id为0，null时，AUTO_INCREMENT填到当前字段当中
- 如果指定则使用指定值
  - 如果指定值比AUTO_INCREMENT大，则对AUTO_INCREMENT进行更新
- 新自增值生成算法：auto_increment_offset 开始，每次叠加auto_increment_increment，直至大于表前的最大值，这两个值的默认值均为1

**修改时机**

在真正数据插入之前，先对AUTO_INCREMENT进行修改，基本插入失败，修改的AUTO_INCREMENT值也不会再回滚

如果进行回滚，则会导致冲突问题，如果当前为2，事务A插入数据改为3,事务A由于唯一键冲突进行回滚，在AUTO_INCREMENT改回2之前事务B插入一条id为3的数据，此时AUTO_INCREMENT的值为2，再重新插入自增数据则导致duplicate key

**自增锁**

语句级别，需要等待该条获取锁的语句执行结束后再释放锁
...
- 1：
  - 普通insert：锁申请之后立马释放
  - 批量插入数据：语句执行结束后再释放(防止其他线程中途插入值，导致binlog在statement的形式下的数据不一致问题) 
    - 解决思路1：语句结束后再释放锁
    - 解决思路2：binlog设置为row形式，innodb_autoinc_lock_mode设置为2,解决数据不一致问题，并且提高并发度
- 2：所有的情况均为申请后立刻释放锁

批量插入数据，包含的语句类型是 insert … select、replace … select 和 load data 语句。innodb_autoinc_lock_mode 设置为 1时，每次申请的id为上一次申请的id的2倍，从而导致了浪费和不连续的问题

**备库自增主键**

由于binlog写入的顺序取决于事务的提交顺序，与真实的执行顺序不一致，因为为了防止由于执行顺序导致的自增主键的值不一样的问题,binlog中在写入插入语句之间回添加一条set INSER_ID = ***,保证不受顺序的影响

**id不连续的原因**

1. 数据插入失败时AUTO_INCREMENT不会回退
2. 事务回滚AUTO_INCRMENT不会回退
3. innodb_autoinc_lock_mode为1时批量申请

### 赋权

grant用于给用户进行赋权，通常无需像部分文档说的那样在后面补充上一个flush privileges,将内存中的改变给flush到硬盘之中

**全局权限**

对于该用户在mysql全局有效，信息保存在mysql的user表当中，通过grant对其进行赋权,给予all privileges最高权限

```sql
grant all privileges on *.* to 'ua'@'%' with grant option;
```

1. 磁盘上将mysql.user表里，用户的所有的对应的权限全改为Y

2. 内存中，从数组acl_users 中找到这个用户对应的对象，将access(权限位)修改为1

通过新的客户端登录该用户时，MySQL创建一个新的连接维护线程对象,通过acl_user进行权限的确认，然后保存到该线程**内部**的权限位，因此该用户已经登录之后，全局权限不受grant的影响

revoke用于撤销权限，作用效果和所作的工作与grant完全相反
...
1. 磁盘上 向mysql.db表中插入一条记录,所有的权限字段全部设置为Y

2. 内存中 增加一个对象到数组acl_dbs中，access权限位全为1

在此之后的权限判断借助于acl_dbs这个全局数组，yinci对应已经登录的用户,grant和revoke都会影响当前的权限状态
...
flush privileges为清空 当前内存中的全局数组(acl_users,acl_dbs),从硬盘的表中重新加载一遍数据，此后再从内存的数组中读取数据时便可读取到最新的权限，达到内存与硬盘的数据一致，但是并不会去更新线程对象中维护的权限，因此对全局权限仍无影响。

在通常情况下，使用grant和revoke可以保证内存和硬盘中的权限对应一致，无需再语句后面跟随flush privileges。

在直接通过删除表中数据的方式进行权限的修改会导致内存中的数据未被更新，此时需要flush privileges来进行硬盘和内存的数据同步

### 自增id上界

**表中的自增的AUTO_INCREMENT**

- 上限根据字段的数据类型而定，像int 即为2^32^-1
- 达到上限后值将不再变化，如果为唯一值则会存在冲突

**InnoDB系统自增的row_id**

-  用于进行提供位置信息(无主键时会生成一个row_id)
- 整个InnoDB系统共享这一个row_id
- 冲突之后则会从0开始，row_id相同的行新数据会把旧数据给覆盖掉
- 上限为2^48^-1

**Xid**

- 对应事务，用于进行redolog 和binlog之间的统一，同一个事务写入binlog和redolog时会使用同一个Xid,从而关联起来
- MySQL内部维护了global_query_id，执行语句时复制给Query_id,事务的第一条语句会将该语句的query_id赋值给事务的Xid
- global_query_id为纯内存变量，随着重启清零，但是重启后会生成新的binlog文件，因此在同一个binlog中不存在Xid冲突的问题
- 上限为2^64^-1
- 理论上存在冲突，实际上难以达到，冲突后从0开始

**InnoDB trx_id**

- 用于判断事务的可见性，通过低水位和高水位来生成活跃数组
- InnoDB内部维护max_trx_id,赋值给新的trx_id
- 由于只读事务不影响事务可见性，因此对于只读事务不分配trx_id
  - 减少申请trx_id的并发度，减小锁冲突
  - 减少活跃事务数组的大小
- 在未分配事务时，trx_id为把当前事务的 trx 变量的指针地址转成整数，再加上 2^48^，
  - 指针地址保证了不会重复，
  - 2^48^将其与正常对可见性有影响的trx_id区分开
- trx_id有时不是按照+1来递增：
  1. update delete涉及到标记删除旧数据，也就是要把数据放到 purge 队列里等待后续物理删除，这个操作也会把 max_trx_id+1， 因此在一个事务中至少加 2；
  2. 后台操作启动内部事务
- max_trx_id持久化存储，重启不会清零，达到上限后清零,清零后导致新的事务可能会到达低水位以下，从而变为可见的事务
- 上限为2^48^-1
- 理论上存在脏读问题，但是达到上限的时间过长

**thread_id**

- 维护了全局变量thread_id_counter
- 上限2^32^-1,到达上限后重置为0
-  存在一个算法来保证不冲突的情况
